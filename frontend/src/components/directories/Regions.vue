<template>
  <div>
    ...
    <v-dialog v-model="addDialog" persistent max-width="800">
      <v-card>
        <v-card-title class="headline grey lighten-2" primary-title>{{'Add region' | translate}}</v-card-title>
          <v-form>
            <v-card-text>
    ...
    <v-dialog v-model="deleteDialog" max-width="500">
      <v-card>
        ...
        <v-card-text>{{'Delete region' | translate}} #{{currentObject.id}} {{currentObject.name}} ?</v-card-text>
        <v-divider></v-divider>
        <v-card-actions>
          <v-spacer></v-spacer>
...
export default {
  ...
  methods: {
    getObjects: function () {
      this.loading = true
      let apiUrl = '/api/region/' + '?page=' + this.pagination.page + '&page_size=' + this.pagination.rowsPerPage
      if (this.search_term) {
    ...
    addObject: function () {
      this.errorMessage = ''
      axios.post(process.env.API_URL + '/api/edit/region/', this.newObject)
        .then(resp => {
          this.loading = false
    ...
    updateObject: function () {
      ...
      if (this.currentPeople !== '') {
        axios.put(process.env.API_URL + '/api/edit/region/' + this.currentObject.id + '/', this.currentObject)
          .then(resp => {
            this.loading = false
    ...
    deleteObject: function () {
      ...
      if (this.currentObject !== '') {
        axios.delete(process.env.API_URL + '/api/edit/region/' + this.currentObject.id + '/')
          .then(resp => {
            this.currentObject = ''
            this.isSelected = false
            this.getObjects()
